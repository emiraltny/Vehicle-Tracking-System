...
Public Class SecurityMngmt

    Private Sub AddButton_Click(sender As Object, e As EventArgs) Handles AddButton.Click
        If SecurityTextBox.Text.Trim = "" Then
            MsgBox("Lütfen güvenlik adı girin!", vbCritical, "Adalı")
            Return
        End If
        Dim counter As Integer = ListView1.Items.Count
        ListView1.Items.Add(counter)
        ListView1.Items(counter).SubItems.Add(SecurityTextBox.Text.Trim)
        SelectLastItem()
        SaveData()
        ListView1.Select()
        SecurityTextBox.Text = ""
        Form1.LoadItems("guvenlikler")
    End Sub
    ...
    Private Sub SaveData()
        Dim savesFolderPath As String = Path.Combine(Application.StartupPath, "guvenlikler")
        ' guvenlikler klasörünün var olup olmadığını kontrol et
        If Not Directory.Exists(savesFolderPath) Then
            ' Klasör yoksa oluştur
            Directory.CreateDirectory(savesFolderPath)
        End If
        Dim filePath As String = Path.Combine(savesFolderPath, "guvenlikler.txt")
    ...
    Private Sub LoadData()
        ...
        If Not Directory.Exists(savesFolderPath) Then
            ...
            Return
        End If
        Dim folderPath As String = Path.Combine(Application.StartupPath, "guvenlikler")
        Dim filePath As String = Path.Combine(folderPath, "guvenlikler.txt")
    ...
    Private Sub SecurityMngmt_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadData()
    End Sub
    ...
    Private Sub SilToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SilToolStripMenuItem.Click
        ...
        If item IsNot Nothing Then
            Dim result As DialogResult = MessageBox.Show(ListView1.SelectedItems(0).SubItems(1).Text + " adlı güvenliği silmek istediğinize emin misiniz?", "Adalı", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
